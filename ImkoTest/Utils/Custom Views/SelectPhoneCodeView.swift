...
class SelectPhoneCodeView:UIView {
    ...
    // MARK: - Views
    let phoneCodeLabel = UILabel(text: "+52", font: .systemFont(ofSize: 16, weight: .medium), textColor: R.color.primaryText())
    private let dropDown = UIImageView(image: R.image.dropDown())
    lazy var phoneStack:UIStackView = {
        let stack = UIStackView(arrangedSubviews: [phoneCodeLabel, dropDown])
        stack.spacing = 12
