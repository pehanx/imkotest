...
class HistoryCollectionView:UICollectionView  {
    ...
    private func setupUI(){
        register(HistoryCell.self, forCellWithReuseIdentifier: HistoryCell.id)
...
class HistoryCell:UICollectionViewCell {
    ...
    func setupData(_ item:CreditHistoryModel) {
        priceLabel.text = "$\(item.price)"
        durationLabel.text = "Por \(item.days) días"
        approvedLabel.text = "Aprobado \(item.approvedDate.toDateString)"
